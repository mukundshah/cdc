...
def comment_identifier():
    return lambda input_str: CommentType.to_string(
        lib.identify_comment_wrapper(input_str.encode("utf-8"))
    )
...
test_cases = [
    ...
    (
        "This is not a comment /** This is a documentation comment */",
        CommentType.INVALID,
    ),
...
def test_mixed_comment_types(comment_identifier):
    assert (
        comment_identifier("// Single line /* Multi-line */") == "Single Line Comment"
    )
...
def test_whitespace_handling(comment_identifier):
    ...
    assert (
        comment_identifier("   /*   Padded multi-line comment   */   ")
        == "Multi Line Comment"
    )
