...
def recognize_pattern(input_str, pattern):
    result = lib.recognize_pattern_wrapper(
        input_str.encode("utf-8"), pattern.encode("utf-8")
    )
...
def test_random_a_star_b_plus(_):
    ...
    input_str = "".join(random.choice("a") for _ in range(a_length)) + "".join(
        random.choice("b") for _ in range(b_length)
    )
    ...
    if random.choice([True, False]):
        ...
        input_str = (
            input_str[:pos]
            + random.choice("cdefghijklmnopqrstuvwxyz")
            + input_str[pos:]
        )
    else:
        # Remove all 'b's
